...
report["tesnifat_table"] = TES_TABLE_READY
...
# 7) Power BI üçün “tek-sheet feed” (YENİ)
#    Burada ən çox rast gəlinən sütun adlarından seçim edilir.
feed_cols = {
    "NV_id":          ["NV qeydiyyat nömrəsi", "NV", "NV_id"],
    "Utilizator":     ["Utilizatorun adı", "Utilizator"],
    "Tesnifat":       ["Təsnifat", "Kod/Təsnifat", "Kod"],
    "Marka":          ["Marka"],
    "Model":          ["Model"],
    "Reng":           ["Rəng", "Reng"],
    "Mebleg_AZN":     ["Məbləğ (AZN)", "Cəmi (AZN)", "Mebleg_AZN"],
    "TesdiqStatus":   ["Təsdiq edici sənədin statusu", "Tesdiq_status"],
    "TehvilStatus":   ["Təhvil-təslim sənədinin statusu", "Tehvil_status"],
    "dt_R":           ["R tarixi", "dt_R"],
    "dt_AB":          ["AB tarixi", "dt_AB"],
    "dt_AF":          ["AF tarixi", "dt_AF"],
}
powerbi_feed = pd.DataFrame()
for out_col, candidates in feed_cols.items():
    for c in candidates:
        if c in df.columns:
            powerbi_feed[out_col] = df[c]
            break

# Tip düzəlişləri
for c in ["dt_R", "dt_AB", "dt_AF"]:
    if c in powerbi_feed.columns:
        powerbi_feed[c] = pd.to_datetime(powerbi_feed[c], errors="coerce")
if "Mebleg_AZN" in powerbi_feed.columns:
    powerbi_feed["Mebleg_AZN"] = pd.to_numeric(powerbi_feed["Mebleg_AZN"], errors="coerce")

report["powerbi_feed"] = powerbi_feed
