...
.card-wrap {background:#ffffff;border-radius:22px;padding:28px 22px;min-height:220px;box-shadow:0 10px 30px rgba(18,59,99,.10);border:1px solid #dbe7f3;display:flex;flex-direction:column;justify-content:center;align-items:center;}
...
.info-box {max-width:900px;margin:80px auto 0 auto;background:#ffffff;border:1px solid #d8e6f3;border-radius:22px;padding:40px 30px;text-align:center;box-shadow:0 10px 30px rgba(18,59,99,.10);}
.info-title {font-size:30px;font-weight:700;color:#123b63;margin-bottom:16px;}
.info-text {font-size:18px;line-height:1.8;color:#4f6478;}
...
.result-box {background:white;border-radius:16px;padding:18px 20px;border:1px solid #dbe7f3;box-shadow:0 6px 18px rgba(18,59,99,.08);margin-bottom:12px;color:#163a5c;font-size:17px;line-height:1.7;}
.block-btn .stButton > button {width:auto !important;padding:10px 20px;}
.small-note {color:#5e7388;font-size:14px;margin-top:-10px;margin-bottom:20px;}
</style>
""", unsafe_allow_html=True)

# ========================= 1-CI BÖLMƏ HELPERS =========================
def _find_one(base: Path, patterns):
    for pat in patterns:
...
def _exec_streamlit_script(path: Path):
    ...
    exec_globals = {"__name__": "__main__", "__file__": str(path)}
    exec(compile("\n".join(filtered), str(path), "exec"), exec_globals)

MODULE1_ROOT = Path(".")
MODULE1_PAGES_DIR = Path("module1_pages") if Path("module1_pages").exists() else Path("pages")
MODULE1_MAIN = _find_one(MODULE1_ROOT, ["0_*.py"])
MODULE1_FILES = {
    "Faylı yüklə / Təmizlə": _find_one(MODULE1_PAGES_DIR, ["1_*.py"]),
    "Arayış paneli": MODULE1_MAIN,
    "Utilizatorlar": _find_one(MODULE1_PAGES_DIR, ["2_*.py"]),
    "Təsnifatlar üzrə": _find_one(MODULE1_PAGES_DIR, ["3_*.py"]),
    "Təsdiqedici statuslar": _find_one(MODULE1_PAGES_DIR, ["4_*.py"]),
    "TT aktlarının statusları": _find_one(MODULE1_PAGES_DIR, ["5_*.py"]),
    "Top-N Ərizəçi": _find_one(MODULE1_PAGES_DIR, ["6_*.py"]),
    "Top-N Marka": _find_one(MODULE1_PAGES_DIR, ["7_*.py"]),
    "Top-N Modellər": _find_one(MODULE1_PAGES_DIR, ["8_*.py"]),
    "Top-N Rəng": _find_one(MODULE1_PAGES_DIR, ["9_*.py"]),
    "Region paylanması": _find_one(MODULE1_PAGES_DIR, ["10_*.py"]),
    "NV yaşları": _find_one(MODULE1_PAGES_DIR, ["11_*.py"]),
    "Export": _find_one(MODULE1_PAGES_DIR, ["12_*.py"]),
    "Parametrlər": _find_one(MODULE1_PAGES_DIR, ["13_*.py"]),
    "QC Yoxlama": _find_one(MODULE1_PAGES_DIR, ["QC_*.py"]),
    "Delta Müqayisə": _find_one(MODULE1_PAGES_DIR, ["Delta_*.py"]),
    "Region xəritəsi": _find_one(MODULE1_PAGES_DIR, ["16_*.py"]),
}
...
def fill_placeholders(doc: Document, lines: List[str]):
    ...
    if len(targets) == 1:
        ...
        for i, p in enumerate(targets[:len(lines)]):
            p.text = ""
            line = lines[i]
...
if st.session_state.page == "home":
    ...
    with col1:
        st.markdown('''<div class="card-wrap"><div class="card-icon">📄</div><div class="card-title">Arayışların Avtomatlaşdırılmış Hazırlanması</div><div class="card-sub">Yükləmə, analiz, export və QC modulları</div></div>''', unsafe_allow_html=True)
        if st.button("Giriş", key="btn1"):
            st.session_state.page = "arayis"
            st.rerun()
    with col2:
        st.markdown('''<div class="card-wrap"><div class="card-icon">🧾</div><div class="card-title">Aktların Avtomatlaşdırılmış Tərtibi</div><div class="card-sub">Excel məlumatlarının Word şablonuna avtomatik yerləşdirilməsi</div></div>''', unsafe_allow_html=True)
        if st.button("Giriş", key="btn2"):
            st.session_state.page = "akt"
            st.rerun()
    with col3:
        st.markdown('''<div class="card-wrap"><div class="card-icon">💳</div><div class="card-title">Maliyyə Ödənişlərinə Nəzarət və Yoxlama Sistemi</div><div class="card-sub">Satış sıralamasına görə say və məbləğin hesablanması</div></div>''', unsafe_allow_html=True)
...
# ========================= 1-Cİ BÖLMƏ =========================
    ...
    missing = []
    if not Path("nvu").exists():
        missing.append("nvu/")
    if not Path("data").exists():
        missing.append("data/")
    if MODULE1_MAIN is None:
        missing.append("0_NVU_Arayış_Paneli.py")
    if not MODULE1_PAGES_DIR.exists():
        missing.append("module1_pages/ (və ya pages/)")

    if missing:
        st.error("1-ci modul üçün bunlar repo-da olmalıdır: " + ", ".join(missing))
    else:
        options = [k for k, v in MODULE1_FILES.items() if v is not None]
        selected = st.selectbox("Bölmə seç", options, index=0)
        try:
            _exec_streamlit_script(MODULE1_FILES[selected])
        except Exception as e:
            st.error(f"1-ci modulda xəta: {e}")
...
# ========================= 2-Cİ BÖLMƏ =========================
    ...
    with colf1:
        excel_file = st.file_uploader("Excel cədvəli (.xlsx)", type=["xlsx"], key="akt_excel")
    with colf2:
        docx_file = st.file_uploader("Word faylı (.docx)", type=["docx"], key="akt_docx")
    ...
    if st.button("AKT yarat və endir", key="akt_go"):
        if not excel_file or not docx_file:
            ...
            try:
                ...
                st.download_button("Docx endir", data=buf.getvalue(), file_name=out_name, mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="akt_download")
            except Exception as e:
                st.error(f"Xəta: {e}")
