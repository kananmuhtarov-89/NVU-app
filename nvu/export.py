...
def _set_table_cell_margins(table, top=80, bottom=80, left=80, right=80):
    try:
        ...
        if cellMar is None:
            cellMar = OxmlElement("w:tblCellMar"); tblPr.append(cellMar)
        def _set(side, val):
            el = cellMar.find(qn(f"w:{side}"))
            if el is None: el = OxmlElement(f"w:{side}"); cellMar.append(el)
            el.set(qn("w:w"), str(val)); el.set(qn("w:type"), "dxa")
        for s, v in [("top",top),("bottom",bottom),("left",left),("right",right)]: _set(s, v)
    except Exception: pass
...
def _add_table(doc: Document, df: pd.DataFrame, add_rownum: bool = False) -> None:
    ...
    table = doc.add_table(rows=1, cols=len(dfx.columns)); table.allow_autofit = True
    _set_table_cell_margins(table); _make_table_borderless(table)
    hdr = table.rows[0].cells
    for i, col in enumerate(dfx.columns):
        _shade_cell(hdr[i]); p = hdr[i].paragraphs[0]; p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    ...
    for _, row in dfx.iterrows():
        ...
        for j, col in enumerate(dfx.columns):
            p = cells[j].paragraphs[0]; p.alignment = WD_ALIGN_PARAGRAPH.LEFT
            p.paragraph_format.space_before = Pt(0); p.paragraph_format.space_after = Pt(0)
...
def export_docx(report: Dict[str, Any], source_filename: str = "") -> bytes:
    ...
    if isinstance(ready, pd.DataFrame) and not ready.empty:
        ...
        if calc and "Cəmi (AZN)" not in t.columns and "Say" in t.columns:
            ...
            if code_col is not None:
                _rates = {"M1":1500,"M2":2000,"M3":3000,"N1":1500,"N2":2000,"N3":3000,"T":2000,"TK":2000,"TT":2000,"H":3000,"HT":3000,"HK":3000,"L":200}
                def _norm(c: str) -> str:
                    s = str(c).strip().upper()
    ...
    # ... (digər bölmələr səndə necədirsə eyni qalsın)
...
def _style_openpyxl_worksheet(ws, df: pd.DataFrame):
    ...
    for idx, col in enumerate(df.columns, start=1):
        max_len = len(str(col))
        for v in df[col].astype(str).values[:500]: max_len = max(max_len, len(v))
